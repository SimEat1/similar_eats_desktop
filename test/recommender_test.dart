...
void main() {
  test("recommender ranks spicy places higher for spicy profile", () {
    final spicy =
        const TasteProfile(sweet: 1, salty: 1.5, sour: 1, spicy: 5, umami: 2.5);
    final recs = Recommender()
        .recommend(profile: spicy, candidates: MockRestaurants.all, topN: 5);
    expect(recs.isNotEmpty, true);
    final top = recs.first.restaurant.cuisine.toLowerCase();
    expect(
        top.contains("thai") || top.contains("mexican") || top.contains("bbq"),
        true);
  });
}
