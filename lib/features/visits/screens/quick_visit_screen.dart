...
import "package:similar_eats_desktop/shared/portion_wire_ext.dart";
...
class _QuickVisitScreenState extends State<QuickVisitScreen> {
  // Optional repo (currently unused but kept for future wiring)
  ...
  bool _dirty = false;
  bool get _canSave => _placeId != null && _itemId != null && _dirty;
  ...
  Future<void> _seedDemoData() async {
    try {
      ...
      if (uid == null || uid.isEmpty) throw Exception("No uid");
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Seeded demo data')));
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
  ...
  Future<void> _pickPlace() async {
    ...
    if (uid == null || uid.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('No user id')));
      return;
    }
  ...
  Future<void> _pickItem() async {
    if (_placeId == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Pick a place first')));
      return;
    }
  ...
  Future<void> _onSavePressed() async {
    debugPrint("[quick_visit] onSavePressed tapped; dirty=$_dirty");
  ...
  Future<void> _saveVisit(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      debugPrint("[quick_visit] _saveVisit entered. uid=$uid");
      ...
      final data = {
        ...
        "portion_size": _portion.wire,
        "taste_rating": payload["taste_rating"],
        "portion_rating": payload["portion_rating"],
        "uid": uid,
        "platform": "windows",
      };
      debugPrint("[quick_visit] final data => $data");

      // owner-scoped path (matches your working rules)
      final docRef = await FirebaseFirestore.instance
          .collection("debug")
          .doc(uid)
          .collection("quick_visits")
          .add(data);

      debugPrint("[quick_visit] saved doc id: ${docRef.id}");
      if (mounted) {
        ScaffoldMessenger.of(context)
      ...
      debugPrint("[quick_visit] save failed: $e");
      debugPrintStack(label: "[quick_visit] save stack", stackTrace: st);
      if (mounted) {
        ScaffoldMessenger.of(context)
